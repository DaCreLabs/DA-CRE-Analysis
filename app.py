...
# =============================================================================
# REALTIME DI VOICE — PHONE-CALL STYLE
# =============================================================================
...
def _openai_api_key():
    """Read the server-side OpenAI API key without ever putting the key in HTML."""
    try:
        value = st.secrets.get("OPENAI_API_KEY", "")
    except Exception:
        value = ""
    return (value or os.getenv("OPENAI_API_KEY", "")).strip()


def create_realtime_client_secret(user, df=None):
    """Create a short-lived browser credential for an OpenAI Realtime WebRTC call."""
    api_key = _openai_api_key()
    if not api_key:
        return None, "OPENAI_API_KEY is not configured. Add it to Streamlit Secrets or the server environment."

    columns = list(df.columns[:30]) if df is not None else []
    dataset_context = (
        f"The user is currently working for {user.get('company','their organization')}. "
        f"Active dataset: {st.session_state.get('active_filename') or 'none'}. "
        f"Visible dataset columns: {', '.join(map(str, columns)) or 'none'}."
    )
    instructions = (
        "You are DI — David's Intelligence, the voice business-intelligence assistant inside DACRE Analysis. "
        "Have a natural, warm, human conversational style, like a helpful professional on a phone call. "
        "Keep answers concise unless the user asks for detail. Do not wait for the user to repeat themselves. "
        "If the user pauses, let them continue. If they start speaking while you are answering, stop speaking and listen to them. "
        "Use natural acknowledgements such as 'yes', 'right', 'okay', or 'I understand' when appropriate, but do not overdo them. "
        "You have access to approved DACRE tools. When a user asks you to perform an operation, call the appropriate tool and report the actual result. Never claim success without a successful tool result. Never invent tool results. "
        + dataset_context
    )
    payload = {
        "session": {
    ...
    try:
        ...
        secret = ((data.get("value") or data.get("client_secret") or {}).get("value")
                  if isinstance(data.get("client_secret"), dict) else data.get("client_secret"))
        if not secret:
            secret = data.get("value")
        if not secret:
            return None, "OpenAI returned no Realtime client secret."
        return str(secret), None
    except Exception as exc:
        return None, f"Could not start DI voice: {type(exc).__name__}: {exc}"


VOICE_COMPONENT_DIR = Path(__file__).resolve().parent / "dacre_realtime_component"
try:
    _dacre_realtime_component = components.declare_component("dacre_realtime_voice", path=str(VOICE_COMPONENT_DIR))
except Exception:
    _dacre_realtime_component = None


# -----------------------------------------------------------------------------
# DACRE VOICE TOOL BRIDGE
# -----------------------------------------------------------------------------
def _voice_tool_specs(user):
    """Tools exposed to Realtime. They map to real DACRE Python operations."""
    tools = [
        {"type":"function","name":"get_workspace_status","description":"Inspect the active DACRE dataset and workspace status.","parameters":{"type":"object","properties":{},"additionalProperties":False}},
        {"type":"function","name":"get_dataset_summary","description":"Return row count, column count, duplicate count, missing values and column names for the active dataset.","parameters":{"type":"object","properties":{},"additionalProperties":False}},
        {"type":"function","name":"run_formula","description":"Run a DACRE Formula Lab operation on the active dataset. Column-changing formulas modify the active dataframe.","parameters":{"type":"object","properties":{"formula":{"type":"string","enum":SHEET_FORMULAS},"column":{"type":"string"},"first":{"type":"string"},"second":{"type":"string"},"new_column":{"type":"string"},"separator":{"type":"string"}},"required":["formula"],"additionalProperties":False}},
        {"type":"function","name":"remove_duplicates","description":"Remove duplicate rows from the active dataset and save the updated workspace state.","parameters":{"type":"object","properties":{},"additionalProperties":False}},
        {"type":"function","name":"create_chart","description":"Create a DACRE chart configuration using columns from the active dataset.","parameters":{"type":"object","properties":{"chart_type":{"type":"string","enum":["Bar Chart","Line Chart","Area Chart"]},"x_column":{"type":"string"},"y_column":{"type":"string"}},"required":["chart_type","x_column","y_column"],"additionalProperties":False}},
        {"type":"function","name":"save_workspace","description":"Save the current DACRE workspace state to the user's project.","parameters":{"type":"object","properties":{},"additionalProperties":False}},
        {"type":"function","name":"list_file_vault","description":"List files stored in the current organization's DACRE File Vault.","parameters":{"type":"object","properties":{},"additionalProperties":False}},
        {"type":"function","name":"load_vault_file","description":"Load a named file from the current organization's File Vault into the active workspace.","parameters":{"type":"object","properties":{"filename":{"type":"string"}},"required":["filename"],"additionalProperties":False}},
        {"type":"function","name":"export_csv","description":"Prepare the current processed dataset for CSV export.","parameters":{"type":"object","properties":{},"additionalProperties":False}},
        {"type":"function","name":"export_excel","description":"Prepare the current processed dataset for Excel export.","parameters":{"type":"object","properties":{},"additionalProperties":False}},
    ]
    if user.get("role") in ("company_admin","master"):
        tools.append({"type":"function","name":"get_organization_accounts","description":"Inspect accounts in the user's organization. Master can inspect the selected organization context.","parameters":{"type":"object","properties":{},"additionalProperties":False}})
    return tools

def execute_voice_tool(name,args,user):
    """Execute only approved DACRE operations; never eval/exec arbitrary model code."""
    args=args or {}
    df=st.session_state.get("processed_df")
    if name=="get_workspace_status":
        return {"ok":True,"active_file":st.session_state.get("active_filename") or None,"rows":len(df) if df is not None else 0,"columns":list(map(str,df.columns)) if df is not None else [],"has_dataset":df is not None}
    if name=="get_dataset_summary":
        if df is None:return {"ok":False,"message":"There is no active dataset."}
        missing={str(k):int(v) for k,v in df.isna().sum().items() if int(v)>0}
        return {"ok":True,"rows":len(df),"columns":len(df.columns),"duplicate_rows":int(df.duplicated().sum()),"missing":missing,"column_names":list(map(str,df.columns))}
    if name=="run_formula":
        if df is None:return {"ok":False,"message":"There is no active dataset."}
        formula=str(args.get("formula","")).upper()
        opts={k:args.get(k) for k in ("column","first","second","new_column","separator") if args.get(k) is not None}
        try:
            if formula=="CONCATENATE":
                if not all(opts.get(k) in df.columns for k in ("first","second")): return {"ok":False,"message":"Both source columns must exist."}
                opts.setdefault("new_column","Combined"); opts.setdefault("separator"," ")
            elif formula in ("SUM","AVERAGE","COUNT","COUNTA","MAX","MIN","UPPER","LOWER","TRIM") and opts.get("column") not in df.columns:
                return {"ok":False,"message":"The requested column does not exist."}
            res=apply_formula(df,formula,opts)
            if isinstance(res,tuple) and res[0]=="column":
                df[res[1]]=res[2]; st.session_state.processed_df=df; st.session_state.formula_logs.append(f"Applied {formula} via DI voice"); save_project(user,st.session_state.raw_df,df,st.session_state.active_filename,st.session_state.formula_logs,st.session_state.chart_config); log_activity(user["username"],user["company"],f"DI voice ran formula {formula}")
                return {"ok":True,"message":f"Applied {formula} successfully.","column":res[1]}
            st.session_state.formula_logs.append(f"{formula}({opts.get('column')}) = {res}"); log_activity(user["username"],user["company"],f"DI voice calculated {formula}"); return {"ok":True,"result":None if pd.isna(res) else res,"formula":formula,"column":opts.get("column")}
        except Exception as exc:return {"ok":False,"message":f"Formula failed: {type(exc).__name__}: {exc}"}
    if name=="remove_duplicates":
        if df is None:return {"ok":False,"message":"There is no active dataset."}
        before=len(df); new=df.drop_duplicates().reset_index(drop=True); removed=before-len(new); st.session_state.processed_df=new; save_project(user,st.session_state.raw_df,new,st.session_state.active_filename,st.session_state.formula_logs,st.session_state.chart_config); log_activity(user["username"],user["company"],f"DI voice removed {removed} duplicate rows"); return {"ok":True,"removed":removed,"remaining":len(new)}
    if name=="create_chart":
        if df is None:return {"ok":False,"message":"There is no active dataset."}
        x,y=args.get("x_column"),args.get("y_column")
        if x not in df.columns or y not in df.columns:return {"ok":False,"message":"One or more requested chart columns do not exist."}
        if not pd.api.types.is_numeric_dtype(df[y]):return {"ok":False,"message":f"{y} is not numeric."}
        st.session_state.chart_config={"type":args.get("chart_type","Bar Chart"),"x":x,"y":y}; save_project(user,st.session_state.raw_df,df,st.session_state.active_filename,st.session_state.formula_logs,st.session_state.chart_config); log_activity(user["username"],user["company"],f"DI voice created {args.get('chart_type')} chart: {x} vs {y}"); return {"ok":True,"chart":st.session_state.chart_config}
    if name=="save_workspace":
        if df is None:return {"ok":False,"message":"There is no active dataset."}
        save_project(user,st.session_state.raw_df,df,st.session_state.active_filename,st.session_state.formula_logs,st.session_state.chart_config); log_activity(user["username"],user["company"],"DI voice saved workspace state"); return {"ok":True,"message":"Workspace saved."}
    if name=="list_file_vault":
        return {"ok":True,"files":[{"filename":r[0],"type":r[1],"created_at":r[2]} for r in get_files(user)]}
    if name=="load_vault_file":
        filename=str(args.get("filename","")).strip(); rows=get_files(user); match=next((r for r in rows if r[0].lower()==filename.lower()),None)
        if not match:return {"ok":False,"message":f"{filename} was not found in the organization File Vault."}
        restored=dataframe_from_json(match[3]);
        if restored is None:return {"ok":False,"message":"The stored file could not be restored."}
        st.session_state.processed_df=restored; st.session_state.raw_df=restored; st.session_state.active_filename=match[0]; log_activity(user["username"],user["company"],f"DI voice loaded file from vault: {match[0]}"); return {"ok":True,"filename":match[0],"rows":len(restored),"columns":len(restored.columns)}
    if name=="export_csv":
        if df is None:return {"ok":False,"message":"There is no active dataset."}
        return {"ok":True,"filename":f"{st.session_state.get('active_filename') or 'dacre'}_processed.csv","bytes":len(df.to_csv(index=False).encode('utf-8')),"message":"CSV data is ready in Export Center."}
    if name=="export_excel":
        if df is None:return {"ok":False,"message":"There is no active dataset."}
        return {"ok":True,"filename":f"{st.session_state.get('active_filename') or 'dacre'}_workbook.xlsx","bytes":len(make_excel(df)) ,"message":"Excel workbook is ready in Export Center."}
    if name=="get_organization_accounts":
        con=db(); rows=con.execute("SELECT first_name,last_name,username,email,role,login_count,last_login FROM users WHERE company_name=? ORDER BY id DESC",(user["company"],)).fetchall(); con.close(); return {"ok":True,"accounts":[dict(r) for r in rows]}
    return {"ok":False,"message":f"Tool {name} is not available to this account."}


def realtime_voice_panel(user, df=None):
    """Bidirectional Realtime component: voice + actual DACRE Python tool execution."""
    token = st.session_state.get("voice_client_secret")
    error = None
    if not token:
        token, error = create_realtime_client_secret(user, df)
        if token:
            st.session_state.voice_client_secret = token
    if error:
        st.warning(error); st.caption("Add OPENAI_API_KEY to Streamlit Secrets and reload the app."); return
    if _dacre_realtime_component is None:
        st.error("The DACRE Realtime component is missing. Keep the dacre_realtime_component folder beside this app.py file."); return
    tool_result=st.session_state.pop("voice_tool_result",None)
    component_value=_dacre_realtime_component(
        client_secret=token,
        user_name=user.get("first_name","there"),
        tools=_voice_tool_specs(user),
        tool_result=tool_result,
        key="dacre_realtime_voice_component",
        default=None,
    )
    if isinstance(component_value,dict) and component_value.get("type")=="tool_call":
        call_id=component_value.get("call_id")
        if call_id and call_id != st.session_state.get("voice_last_tool_call_id"):
            st.session_state.voice_last_tool_call_id=call_id
            try: args=json.loads(component_value.get("arguments") or "{}")
            except Exception: args={}
            result=execute_voice_tool(component_value.get("name",""),args,user)
            st.session_state.voice_tool_result={"type":"tool_result","call_id":call_id,"result":result}
            st.rerun()
...
for key, default in {
    ...
    "master_second_attempt": False, "voice_client_secret": None, "voice_last_tool_call_id": None,
}.items():
...
def landing_page():
    ...
    <style>
      #dacre-ceo-building-access-v2:hover{
        ...
        box-shadow:0 28px 72px rgba(23,105,170,.34);
...
st.markdown("""
<style>
:root{--di-blue:#1769aa;--di-blue-soft:#edf5ff;--di-blue-line:#c9d9ef;--di-ink:#17202b;--di-muted:#657180;--di-shadow:0 18px 55px rgba(24,72,130,.10)}
.stApp{background:linear-gradient(180deg,#ffffff 0%,#f7fbff 52%,#edf5ff 100%) !important;color:var(--di-ink) !important}
...
[data-testid="stSidebar"]{background:#ffffff !important;border-right:1px solid var(--di-blue-line) !important;box-shadow:8px 0 30px rgba(24,72,130,.04)}
...
[data-testid="stSidebar"] [data-testid="stRadio"] label:hover{background:var(--di-blue-soft);transform:translateX(3px)}
.stButton>button{border:1px solid var(--di-blue-line)!important;background:#fff!important;color:var(--di-ink)!important;border-radius:12px!important;transition:.22s ease!important;box-shadow:0 5px 18px rgba(24,72,130,.05)!important}
.stButton>button:hover{border-color:var(--di-blue)!important;background:var(--di-blue-soft)!important;transform:translateY(-2px);box-shadow:0 10px 28px rgba(23,105,170,.16)!important}
.stTextInput input,.stTextArea textarea,.stSelectbox div[data-baseweb="select"]>div{background:#fff!important;border:1px solid var(--di-blue-line)!important;color:var(--di-ink)!important;border-radius:12px!important}
.dacre-user-hero{background:linear-gradient(115deg,#fff,#f2f7ff);border:1px solid var(--di-blue-line);border-radius:24px;padding:24px 28px;box-shadow:var(--di-shadow)}
...
.di-command{background:linear-gradient(135deg,#fff,#edf5ff);border:1px solid #c9d9ef;border-radius:26px;box-shadow:var(--di-shadow);overflow:hidden;position:relative}
...
.di-orb{position:absolute;right:9%;top:18%;width:170px;height:170px;border-radius:50%;background:radial-gradient(circle at 35% 30%,#fff,#9bc4ea 35%,#1769aa 68%,rgba(23,105,170,0) 70%);box-shadow:0 0 80px rgba(23,105,170,.34);animation:diPulse 4s ease-in-out infinite}
...
@keyframes diPulse{50%{transform:scale(1.07);box-shadow:0 0 105px rgba(23,105,170,.42)}}
@keyframes diSpin{to{transform:rotate(360deg)}}
.di-stage-copy{position:absolute;left:30px;top:30px;max-width:58%}
.di-kicker{font-size:.76rem;letter-spacing:.16em;text-transform:uppercase;font-weight:800;color:#1769aa!important}
...
.di-status{display:inline-flex;align-items:center;gap:8px;padding:7px 11px;border-radius:999px;background:#fff;border:1px solid var(--di-blue-line);font-size:.82rem;font-weight:700}
.di-dot{width:8px;height:8px;border-radius:50%;background:#36b37e;box-shadow:0 0 0 5px rgba(54,179,126,.12)}
.di-transcript{padding:18px 22px;background:#fff;border-top:1px solid var(--di-blue-line);min-height:92px}
.di-transcript-label{font-size:.72rem;letter-spacing:.12em;text-transform:uppercase;color:#54718e!important;font-weight:800}
.di-transcript-text{font-size:1rem;line-height:1.55;margin-top:4px}
.di-quick-card{height:100%;background:#fff;border:1px solid var(--di-blue-line);border-radius:18px;padding:18px;transition:.2s ease;box-shadow:0 10px 30px rgba(24,72,130,.06)}
.di-quick-card:hover{transform:translateY(-4px);box-shadow:0 18px 40px rgba(232,106,168,.12);border-color:#9fc3e5}
.di-metric{background:#fff;border:1px solid var(--di-blue-line);border-radius:16px;padding:16px 18px;box-shadow:0 8px 25px rgba(24,72,130,.06)}
...
def di_voice_bridge():
    """Compatibility wrapper retained for older callers; the DI Home page now uses WebRTC."""
    return None
...
if selected_page=="DI Home":
    ...
    # Phone-call style WebRTC voice. The browser connects directly to the Realtime session
    # using a short-lived client secret created server-side from OPENAI_API_KEY.
    realtime_voice_panel(user, st.session_state.processed_df)
    ...
    for col,(title,headline,desc) in zip([q1,q2,q3,q4],cards):
        with col:
            st.markdown(f"<div class='di-quick-card'><div style='font-size:.75rem;letter-spacing:.1em;text-transform:uppercase;color:#1769aa!important;font-weight:800'>{title}</div><h4 style='margin:.45rem 0'>{headline}</h4><p style='color:#657180!important;font-size:.9rem;line-height:1.45'>{desc}</p></div>",unsafe_allow_html=True)
    ...
    st.caption("Voice mode is a real WebRTC speech-to-speech call: DI can listen continuously, pause naturally, interrupt its own response when you speak, and let you pause/resume your microphone without ending the call.")
...
# Realtime voice audio is delivered directly by the WebRTC call.
# Do not use browser speechSynthesis here; it cannot provide true interruption-aware dialogue.
