...
st.markdown("""
<style>
:root{--blue:#1f5fae;--blue2:#2f78c4;--navy:#12365a;--sky:#eaf3fb;--line:#c9d9e8;--text:#17324d;--muted:#60758a;--orange:#d9822b}
.stApp{background:#f4f8fc!important;color:var(--text)!important}
.main .block-container{max-width:1500px;padding-top:1.25rem;padding-bottom:4rem}
.stApp p,.stApp span,.stApp label,.stApp li,.stApp td,.stApp th,.stApp h1,.stApp h2,.stApp h3,.stApp h4,.stApp h5,.stApp h6{color:var(--text)!important}
.stApp p,.stApp li,.stApp td,.stApp th{line-height:1.55}
[data-testid="stSidebar"]{background:#ffffff!important;border-right:1px solid var(--line)!important;box-shadow:5px 0 22px rgba(18,54,90,.07)}
[data-testid="stSidebar"] *{color:var(--text)!important}
[data-testid="stSidebar"] [data-testid="stRadio"] label{border-radius:10px;padding:8px 10px;font-weight:700}
[data-testid="stSidebar"] [data-testid="stRadio"] label:hover{background:#eaf3fb;transform:translateX(2px);box-shadow:inset 3px 0 0 var(--blue)}
.stButton>button,.stFormSubmitButton>button,.stDownloadButton>button{border:1px solid #b7cde2!important;background:var(--blue)!important;color:#fff!important;border-radius:10px!important;font-weight:750!important;box-shadow:0 4px 12px rgba(31,95,174,.14)!important}
.stButton>button:hover,.stFormSubmitButton>button:hover,.stDownloadButton>button:hover{background:#174f94!important;border-color:#174f94!important;transform:translateY(-1px)}
.stTextInput input,.stTextArea textarea,.stNumberInput input,.stDateInput input,.stSelectbox div[data-baseweb="select"]>div{background:#fff!important;border:1px solid #b9ccdf!important;color:var(--text)!important;border-radius:9px!important;font-weight:600!important}
.stTextInput input::placeholder,.stTextArea textarea::placeholder{color:#8395a7!important}
[data-baseweb="popover"],[data-baseweb="menu"]{background:#fff!important}
[data-baseweb="option"]{color:var(--text)!important}
[data-baseweb="option"]:hover{background:#eaf3fb!important}
.dacre-hero,.dacre-user-hero{background:#fff;border:1px solid var(--line);border-top:4px solid var(--blue);border-radius:16px;padding:22px 26px;box-shadow:0 8px 24px rgba(18,54,90,.08);margin-bottom:18px}
.dacre-title,.dacre-user-title{font-size:2.25rem;font-weight:900;letter-spacing:-.035em;color:var(--navy)!important}.dacre-sub,.dacre-user-sub{color:var(--muted)!important;font-size:1rem}
.di-command{background:#fff;border:1px solid var(--line);border-radius:18px;box-shadow:0 10px 28px rgba(18,54,90,.09);overflow:hidden}
.di-stage{height:300px;position:relative;overflow:hidden;background-size:cover;background-position:center}.di-stage-overlay{position:absolute;inset:0;background:linear-gradient(90deg,rgba(10,35,60,.92),rgba(20,71,112,.72),rgba(10,35,60,.20))}
.di-stage-copy{position:absolute;left:28px;top:28px;max-width:64%}.di-stage-copy h2{font-size:2rem;margin:.45rem 0;color:#fff!important}.di-stage-copy p{color:#e7f0f8!important}.di-kicker{font-size:.75rem;letter-spacing:.14em;text-transform:uppercase;font-weight:800;color:#ffd3a8!important}.di-status{display:inline-flex;align-items:center;gap:8px;padding:6px 10px;border-radius:999px;background:rgba(255,255,255,.14);border:1px solid rgba(255,255,255,.3);color:#fff!important}.di-status *{color:#fff!important}.di-dot{width:8px;height:8px;border-radius:50%;background:#42c77a}.di-orb{position:absolute;right:10%;top:22%;width:145px;height:145px;border-radius:50%;background:radial-gradient(circle,#fff,#c9e8ff 35%,#4d8dcc 65%,rgba(77,141,204,0) 70%);box-shadow:0 0 55px rgba(76,140,204,.32)}
.di-transcript{padding:16px 20px;background:#f7fafd;border-top:1px solid var(--line);min-height:82px}.di-transcript-label{font-size:.72rem;letter-spacing:.11em;text-transform:uppercase;color:var(--blue)!important;font-weight:800}.di-transcript-text{margin-top:4px;color:var(--text)!important}
.di-quick-card,.di-metric,.voice-panel{background:#fff;border:1px solid var(--line);border-radius:14px;padding:16px;box-shadow:0 5px 16px rgba(18,54,90,.06)}
.di-quick-card:hover{border-color:#8fb4d5;box-shadow:0 9px 22px rgba(18,54,90,.10)}
.di-metric .v{font-size:1.5rem;font-weight:850;color:var(--navy)!important}.di-metric .l{font-size:.78rem;color:var(--muted)!important}
.master-office-hero{background:linear-gradient(120deg,#12365a,#1f5fae);border:1px solid #8fb4d5;border-left:7px solid var(--orange);border-radius:18px;padding:25px 29px;box-shadow:0 10px 30px rgba(18,54,90,.12);margin-bottom:18px}.master-office-hero .title{font-size:2.7rem;font-weight:900;color:#fff!important}.master-office-hero .sub{color:#e8f2fb!important}.master-office-hero .authority,.master-only-badge{display:inline-block;margin-top:12px;padding:6px 11px;border-radius:999px;background:#fff3e6;border:1px solid #e3ad77;color:#8a4e18!important;font-weight:800}.master-only-badge{margin-top:0}
.chat-card{padding:14px 17px;border-radius:13px;border:1px solid var(--line);background:#fff;margin:8px 0}.chat-card.di{border-left:4px solid var(--blue);background:#edf5fc}.chat-card.user{border-left:4px solid #8b6fb3}
[data-testid="stDataFrame"]{border:1px solid var(--line);border-radius:10px;overflow:hidden}.stCaption,[data-testid="stCaptionContainer"]{color:var(--muted)!important}
[data-testid="stMetric"]{background:#fff!important;border:1px solid var(--line)!important;border-radius:12px!important}.stMetricValue,[data-testid="stMetricValue"]{color:var(--navy)!important}.stMetricLabel,[data-testid="stMetricLabel"]{color:var(--muted)!important}
[data-testid="stExpander"]{background:#fff!important;border:1px solid var(--line)!important;border-radius:10px!important}
...
# =============================================================================
# FINAL LIGHT BLUE APPLICATION THEME
# =============================================================================
...
if selected_page=="DI Home":
    ...
    with vc2:
        ...
        if st.button("🔊 Test DI Voice",use_container_width=True):
            st.session_state.last_speech = "Hello. I am DI, David's Intelligence. My voice system is ready. You can speak to me now."
            st.rerun()
    with vc3:
        st.markdown("<div class='voice-panel'><b>🎙️ Natural conversation</b><br><span style='font-size:.84rem;color:#49677f!important'>Allow microphone access in Chrome. Speak after the voice bridge starts; DI will answer and the browser will read the answer aloud.</span></div>",unsafe_allow_html=True)
...
# =============================================================================
    ...
    con=db()
    tabs=st.tabs(["Executive Overview","DI Workforce","Organizations","People & Accounts","Chibobec Access","Live Activity","DI Conversations","DI Memory Box","Mail Source","System Controls"])
    ...
    with tabs[0]:
        ...
        with right:
            ...
            if st.button("Refresh executive view",use_container_width=True):
                st.rerun()
            chibo_overview = chibobec_login_monitor()
            st.markdown("#### Chibobec access")
            if chibo_overview.empty:
                st.info("No Chibobec account is recorded in this database yet.")
            else:
                st.success(f"{len(chibo_overview)} Chibobec account(s) recorded. Latest login: {chibo_overview.iloc[0]['last_login'] or 'never recorded'}")
    ...
    with tabs[4]:
        st.subheader("Chibobec Access & Login Monitor")
        st.caption("This is the direct Chibobec account monitor. It shows whether a Chibobec account exists, how many times it has signed in, and the latest recorded login time.")
        chibo_df = chibobec_login_monitor()
        if chibo_df.empty:
            st.warning("No Chibobec account has been recorded in this application's database yet. If the client used a different deployment or the app database was reset, that login will not appear here.")
        else:
            c1,c2,c3=st.columns(3)
            c1.metric("Chibobec accounts", len(chibo_df))
            c2.metric("Accounts with a login", int((chibo_df["login_status"] == "Logged in").sum()))
            c3.metric("Total recorded logins", int(pd.to_numeric(chibo_df["login_count"], errors="coerce").fillna(0).sum()))
            st.dataframe(safe_dataframe_for_streamlit(chibo_df[["first_name","last_name","email","role","login_count","created_at","last_login","login_status"]]), use_container_width=True, hide_index=True)

            recent_chibo = pd.read_sql_query(
                "SELECT username, company_name, action, created_at FROM activity WHERE lower(company_name) LIKE '%chibobec%' ORDER BY id DESC LIMIT 50",
                db(),
            )
            st.markdown("#### Chibobec activity")
            st.dataframe(safe_dataframe_for_streamlit(recent_chibo), use_container_width=True, hide_index=True)
