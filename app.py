...
def _landing_asset_data_uri(filename):
    """Return a bundled landing-page image as a browser-safe data URI."""
    try:
        path = DACRE_ASSET_DIR / filename
        if not path.exists():
            return ""
        mime = "image/png"
...
def landing_page():
    ...
    if current_section == "features":
        ...
        # Premium DI showcase supplied by the DACRE owner. These are bundled locally
        # so the landing page remains self-contained on Streamlit Cloud and does not
        # depend on an external image host.
        showcase_01 = _landing_asset_data_uri("landing_di_showcase_01.png")
        showcase_02 = _landing_asset_data_uri("landing_di_showcase_02.png")
        if showcase_01 or showcase_02:
            st.markdown(
                """
                <style>
                  .di-showcase-wrap{{
                    margin:22px 28px 8px; padding:1px; border-radius:30px;
                    background:linear-gradient(135deg,rgba(47,125,255,.62),rgba(73,226,255,.20),rgba(139,92,246,.38));
                    box-shadow:0 35px 100px rgba(0,0,0,.46);
                  }}
                  .di-showcase{{
                    border-radius:29px; padding:28px; overflow:hidden; position:relative;
                    background:radial-gradient(circle at 78% 0%,rgba(42,126,255,.18),transparent 34%),
                               linear-gradient(145deg,#07101c,#04070d 65%,#091321);
                    border:1px solid rgba(255,255,255,.08);
                  }}
                  .di-showcase-head{{display:flex;justify-content:space-between;align-items:flex-end;gap:20px;flex-wrap:wrap;margin-bottom:22px}}
                  .di-showcase-kicker{{font-size:10px;letter-spacing:.19em;text-transform:uppercase;font-weight:900;color:#61c7ff}}
                  .di-showcase-title{{margin:7px 0 0;color:#fff;font-size:clamp(30px,4.5vw,56px);line-height:.98;letter-spacing:-.055em;font-weight:950;text-shadow:0 8px 30px rgba(0,0,0,.48)}}
                  .di-showcase-copy{{max-width:560px;color:#9db0c8;font-size:14px;line-height:1.7;margin:0}}
                  .di-showcase-status{{display:inline-flex;align-items:center;gap:8px;padding:8px 12px;border-radius:999px;border:1px solid rgba(80,226,179,.25);background:rgba(34,190,133,.08);color:#79edc1;font-size:10px;font-weight:900;letter-spacing:.08em;white-space:nowrap}}
                  .di-status-dot{{width:7px;height:7px;border-radius:50%;background:#53e6ad;box-shadow:0 0 15px #53e6ad}}
                  .di-showcase-grid{{display:grid;grid-template-columns:1.12fr .88fr;gap:16px;align-items:stretch}}
                  .di-shot{{position:relative;overflow:hidden;border-radius:22px;border:1px solid rgba(126,160,210,.20);background:#050912;box-shadow:0 25px 60px rgba(0,0,0,.38)}}
                  .di-shot img{{display:block;width:100%;height:100%;object-fit:cover;transition:transform .7s ease,filter .7s ease}}
                  .di-shot:hover img{{transform:scale(1.025);filter:saturate(1.08)}}
                  .di-shot-main{{min-height:440px}}
                  .di-shot-side{{min-height:440px}}
                  .di-shot-main img{{aspect-ratio:3/2;object-fit:cover}}
                  .di-shot-side img{{aspect-ratio:3/2;object-fit:cover}}
                  .di-shot-glow{{position:absolute;inset:auto 18px 18px 18px;padding:12px 14px;border-radius:15px;
                    background:linear-gradient(135deg,rgba(5,10,18,.90),rgba(9,24,42,.78));backdrop-filter:blur(12px);
                    border:1px solid rgba(255,255,255,.10);box-shadow:0 14px 34px rgba(0,0,0,.35)}}
                  .di-shot-glow strong{{display:block;color:#fff;font-size:12px;letter-spacing:.02em}}
                  .di-shot-glow span{{display:block;color:#91a6bf;font-size:10px;margin-top:4px;line-height:1.45}}
                  .di-showcase-features{{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-top:16px}}
                  .di-showcase-feature{{padding:15px;border:1px solid rgba(126,160,210,.15);border-radius:16px;background:rgba(255,255,255,.035)}}
                  .di-showcase-feature b{{display:block;color:#f5f9ff;font-size:12px}}
                  .di-showcase-feature span{{display:block;color:#7f93ad;font-size:10px;margin-top:5px;line-height:1.4}}
                  @media(max-width:900px){{.di-showcase-grid{{grid-template-columns:1fr}}.di-shot-main,.di-shot-side{{min-height:auto}}.di-showcase-features{{grid-template-columns:1fr 1fr}}}}
                  @media(max-width:680px){{.di-showcase-wrap{{margin:18px 10px 6px}}.di-showcase{{padding:18px}}.di-showcase-title{{font-size:38px}}.di-showcase-features{{grid-template-columns:1fr}}}}
                </style>
                <section class="di-showcase-wrap">
                  <div class="di-showcase">
                    <div class="di-showcase-head">
                      <div>
                        <div class="di-showcase-kicker">Meet the intelligence layer</div>
                        <div class="di-showcase-title">DI. Present. Intelligent. Built for business.</div>
                        <p class="di-showcase-copy">See the DACRE DI experience as it is meant to feel: a visible intelligence partner that can explain performance, surface opportunities and guide the next decision.</p>
                      </div>
                      <div class="di-showcase-status"><span class="di-status-dot"></span> DI EXPERIENCE ONLINE</div>
                    </div>
                    <div class="di-showcase-grid">
                      {main_card}
                      {side_card}
                    </div>
                    <div class="di-showcase-features">
                      <div class="di-showcase-feature"><b>Business Intelligence</b><span>Turn business signals into clear explanations.</span></div>
                      <div class="di-showcase-feature"><b>AI Analysis</b><span>Ask questions and work from the available context.</span></div>
                      <div class="di-showcase-feature"><b>Growth Strategy</b><span>Move from insight to an actionable next step.</span></div>
                      <div class="di-showcase-feature"><b>Smarter Decisions</b><span>Keep the intelligence layer close to the work.</span></div>
                    </div>
                  </div>
                </section>
                """.format(
                    main_card=(
                        f'<div class="di-shot di-shot-main"><img src="{showcase_01}" alt="DACRE DI business intelligence showcase" />'
                        '<div class="di-shot-glow"><strong>DACRE DI · Real business conversation</strong><span>Face, workspace and intelligence controls presented together.</span></div></div>'
                        if showcase_01 else ''
                    ),
                    side_card=(
                        f'<div class="di-shot di-shot-side"><img src="{showcase_02}" alt="DACRE DI assistant on laptop" />'
                        '<div class="di-shot-glow"><strong>Meet DI anywhere</strong><span>A premium visual identity for the intelligence experience.</span></div></div>'
                        if showcase_02 else ''
                    ),
                ),
                unsafe_allow_html=True,
