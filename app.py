...
# -----------------------------------------------------------------------------
# 1. APP CONFIGURATION & PERMANENT LOGO FAVICON LOCKER
# -----------------------------------------------------------------------------
...
# Load image object for Streamlit config
page_icon_obj = "📊"
...
# PERMANENT FAVICON LOCKER (OVERWRITES AND STOPS STREAMLIT'S RED CIRCLE)
if os.path.exists(LOGO_PATH):
    try:
        ...
        favicon_js = f"""
            <script>
                (function lockFaviconPermanently() {{
                    const logoUri = 'data:image/png;base64,{encoded_logo}';
                    ...
                    function applyFavicon() {{
                        try {{
                            ...
                            // Remove default Streamlit favicons / red running circle
                            ...
                            // Attach custom logo if not already present
                            if (!doc.querySelector("link[href='" + logoUri + "']")) {{
                                var newLink = doc.createElement('link');
                                newLink.type = 'image/png';
                                newLink.rel = 'shortcut icon';
                                newLink.href = logoUri;
                    ...
                    // MutationObserver ensures red circle is instantly killed if Streamlit attempts to inject it
                    try {{
                        var target = window.parent.document.querySelector('head');
                        if (target) {{
